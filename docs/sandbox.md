...
coda-sandbox clone <env> <new-name>
coda-sandbox list
coda-sandbox install <env> <package> [more packages…]
coda-sandbox shell <env>
coda-sandbox exec <env> <command> [params…]
coda-sandbox help
```

| Command | What it does |
| --- | --- |
| `create <env>` | Bootstrap `base` + `iptables` (`--noconfirm --needed`) in a user namespace. Env is required. |
| `destroy <env>` | Delete that sandbox (no prompt; `--force` is a no-op) |
| `clone <env> <new-name>` | Copy an existing sandbox |
| `list` | Inventory of named sandboxes |
| `install <env> <package> […]` | `pacman --noconfirm -S` into that env. Env and at least one package are required. |
| `shell <env>` | `bwrap` with the sandbox as `/`, then a login shell |
| `exec <env> <command> […]` | Same namespace, then `command` |
| `help` | This command list |

There is no default env name, and no `path` or `pacman` subcommand. Persist database files (or other state you must keep after `destroy`) with `--bind HOST:GUEST` or by keeping them under `$HOME` (bound unless `--no-home`).
...
coda-sandbox shell dev
coda-sandbox exec dev postgres --version
coda-sandbox clone dev experiment
coda-sandbox list
...
A named sandbox is **one Arch root**, not one app. `~/.coda/sandbox/<env>` can hold `postgresql`, `redis`, `git`, and anything else you `coda-sandbox install <env> …` into it. Create a second name only when you want a **separate** disposable root (different experiment, throwaway vs long-lived). Do not assume a 1:1 app↔sandbox mapping.

## Layout

Documented path: **`~/.coda/sandbox/<env>`** (singular `sandbox`). The env directory is the sandbox; the pacman tree lives inside it:
...
`shell` / `exec` bind the sandbox as `/`, plus `/proc`, `/dev`, read-only `/sys` (try), host `resolv.conf`, `~/.coda/cache/pacman` at `/var/cache/pacman/pkg` inside the sandbox, and `$HOME` by default. Sandbox `/etc/resolv.conf` is written as a **regular file** (never a systemd symlink into `/run`); otherwise `bwrap --ro-bind` fails after `--tmpfs /run`.
...
After bootstrap, `coda-sandbox` adds `/etc/os-release` → `../usr/lib/os-release`. The `filesystem` package only ships `usr/lib/os-release`; on a real host the `/etc` symlink comes from systemd-tmpfiles (`L /etc/os-release`), which `pacman --root` does not run. `shell`/`exec` recreate the link if it is still missing.
...
- `shell`/`exec` stay as the real user (no uid remap) so those user-owned files stay writable.
