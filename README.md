...
The live ISO autologins user `live` into Hyprland on tty1 (`coda-hyprland` → `start-hyprland`, VirtualBox software-render path). Locale/timezone/keymap are Bozeman, Montana defaults (`en_US.UTF-8`, `America/Denver`, `us`) and are not asked at install time.
