...
`system-config-report` (the ISO user unit) serves scan RPCs **and** watches `NETLINK_KOBJECT_UEVENT` by default (`--no-watch` to disable; `--watch` for watch-only). Mapped paths: net→network, usb/pci, bluetooth, input, backlight, drm→display. A **slow poll** (default 30s) remains for L2 stacks that do not emit kobject uevents: Hyprland scale/mode, PipeWire volume, timedatectl/locale, logind session. Hidden SSIDs: desired `wifi.hidden` → `iwctl station … connect-hidden`.

Still open:
- iwd scan quality; networkd drop-in merge with existing `*.network` files.
