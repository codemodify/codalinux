...
| Network / audio / bluetooth / input / datetime / locale / session / power / printers / users / storage | Same two-pane + `TableView` / `TextField` / `Switch` / `Slider`. No PrefsPage. Bluetooth PIN is an `Overlay` + `NewPanel` dialog (`Pair…`). |
