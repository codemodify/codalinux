...
`ldconfig.service` is on `sysinit.target` (~30s on QEMU when it runs). Stock conditions are an **OR**: `ConditionNeedsUpdate=/etc` **or** missing `/etc/ld.so.cache`. Live overlay boots usually have a cache already (glibc hook during mkarchiso) but **no** `/etc/.updated`, so systemd treats `/etc` as stale and rebuilds the cache before greetd. Drop-in `ldconfig.service.d/coda.conf` resets `ConditionNeedsUpdate=` so the unit only runs if the cache file is empty. `root/customize_airootfs.sh` (when mkarchiso still invokes it) runs `ldconfig -X`, `locale-gen en_US.UTF-8`, and `touch /etc/.updated`.
