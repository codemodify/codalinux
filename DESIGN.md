...
| Clients | `system-config` (CLI), `system-config-tui` (every KnownPath; per-section Apply), `system-config-gui` (uitoolkit Mail/Settings; TreeView per domain). All talk to D only. |
