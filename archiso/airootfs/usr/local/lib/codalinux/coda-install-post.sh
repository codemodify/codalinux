#!/usr/bin/env bash
# Copy the live Coda desktop onto an installed slot and enable greetd
# for the installed user (default: user). Runs from the live ISO after
# the offline A/B copy (coda-install-ab.sh / coda-slot). Do not rely on
# arch-chroot custom_commands — those cannot see live /usr/local.
set -euo pipefail
